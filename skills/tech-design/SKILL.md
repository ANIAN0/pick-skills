...
**规则**：
- 功能点编号全局唯一（FP-N），后续阶段引用
- 每个功能点对应一种测试方式：后端脚本 或 前端审查
- 需求中的每个 `[后端]` 标记至少产生一个后端脚本功能点
- 需求中的每个 `[前端]` 标记至少产生一个前端审查功能点
...
|-----------|-----------|------------------|-------------|
| TC-1 | FP-1 | test/backend/fp1_test.py | POST /api/tickets有效数据，返回201，数据库有记录 |
| TC-2 | FP-2 | test/backend/fp2_test.py | POST /api/tickets空标题，返回400，数据库无记录 |
| TC-3 | FP-3 | test/backend/fp3_test.py | POST /api/tickets重复标题，返回409 |
| TC-4 | FP-4 | review/fp4_checklist.md | 成功Toast、跳转列表、列表有新数据 |
| TC-5 | FP-5 | review/fp5_checklist.md | 错误标红、不跳转、按钮恢复 |
| TC-6 | FP-6 | review/fp6_checklist.md | 按钮loading、disabled、提示文字 |

**规则**：
- 测试点编号全局唯一（TC-N）
- 后端测试点 = 一个可执行的py脚本路径（计划阶段编写具体脚本）
- 前端测试点 = 一个审查清单文件路径（计划阶段编写具体清单）
- 技术方案阶段只定义路径和内容摘要，不编写具体脚本/清单内容
