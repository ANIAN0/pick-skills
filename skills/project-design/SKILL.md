...
5. 对照目标项目当前实现和目标依赖版本，逐项记录“保留、适配、替换、排除或受阻”及理由；
6. 将每项纳入范围的源行为或必要支撑映射到需求位置、方案机制、负责模块和可观察验证点。
...
无法运行或读取参考实现时，把缺失证据、已采用的交叉验证和它对设计结论的影响记录为受阻项。关键用户链路、核心规则或生命周期边界仍无法确认时，停止对应方案结论，不能用“后续实现时再确认”或用户确认代替源调查。
...
2. **参考迁移完整性**：存在指定参考实现时，迁移基线中的每项源行为、核心规则和必要支撑是否已有目标处置、方案承接和版本差异依据；
...
方案阶段完成不表示已经证明不存在未知问题。只有规定的调查和审查已经执行、证据基线覆盖全部受影响链路、指定参考实现的迁移基线已经确认且每项范围内容都有目标处置、每项需求与保留能力都有真实方案承接、独立生命周期和真实失败边界已经闭合、验证层级足以证明交付行为、候选问题均已查证分类、实质问题已经解决，并且用户已经审阅和明确认可当前正文，才将正文标记为已确认并允许交给规划阶段。
