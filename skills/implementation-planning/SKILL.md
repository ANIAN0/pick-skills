...
1. 从 `workplace/1.X/tech-design/` 读取技术方案（含前端设计）
2. 提取架构、数据模型、API、前端设计 → 自主拆分模块
3. 默认顺序：**数据层 → 服务层 → 接口层 → 前端基础（路由/状态/API 客户端） → 前端页面**
4. 关联验收标准，测试文件位置统一指向 `workplace/1.X/test/` 下对应子目录（单元 + 必要集成；不含 e2e）
5. 生成下方模板，提交用户确认
