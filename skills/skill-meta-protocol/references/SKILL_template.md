...
<通用部分默认遵循 skill-meta-protocol 的提问契约；只在这里写特殊于通用规则的差异部分，例如"本 skill 在 X 阶段绝对不能提问"或"必须批量收集问题"。>

## 证据与完成定义

- 完成前必须保留的证据：<命令行输出 / 文件链接 / 日志片段 / 验证报告>
- 标记 `done` 的前提：<参见"校验与门槛"模块，不重复定义判据>
- 额外的证据要求：<如有，写在这里；通用规则参见 skill-meta-protocol>
...
<扩张边界时检查是否与另一个 skill 的边界产生重叠，参见 skill-meta-protocol 的"改动影响自检表"。>
