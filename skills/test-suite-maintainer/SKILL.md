...
3. **逐条执行核心流程** — 对每个核心功能点，确认完整链路跑通：请求发出 → 后端处理 → 数据写入/外部调用 → 响应正确返回
4. **标记问题** — 任何流程测试失败 = 上线阻断，不得以"单元测试通过"绕过
5. **生成验证报告** — 记录哪些主流程已验证通过，哪些有问题
...
8. **核心功能要有集成测试** - 对 priority: high 的功能，尽量维护 type: integration 或 type: e2e 的用例，而不是只有 unit；单元测试通过不等于真实环境跑得通
9. **上线前验证走场景四** - 上线前除了跑场景一自动化套件，还要用场景四确认核心主流程在真实环境下端到端跑通；如果 manifest 中高优先级用例全是 unit，说明集成验证覆盖缺失，应先补充