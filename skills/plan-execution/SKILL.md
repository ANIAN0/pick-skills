...
- 计划文件路径：`workplace/1.X/plan/`（X 为当前活跃版本数字）
- **所有新增测试文件必须落到 `workplace/1.X/test/` 下对应子目录**（backend/unit、backend/integration、frontend/unit、frontend/component、fixtures），不得放在源码旁
- 执行过程中**不提交 git**
</HARD-GATE>
...
模块状态以**计划文件中的状态列**为准，不依赖内存（会话中断即丢失）。
