...
- 需要判断业务知识是否应沉淀到 `project-kb/`，该判断交给负责 `project-kb/` 的 skill。
...
1. **抽**：调起负责维护 `project-kb/` 的 skill 的 archive intake 入口；该 skill 自己决定抽取哪些内容到 `project-kb/`。本 skill 不参与判定。若该 skill 不可用，报告该步骤跳过的原因，再继续清和移。
